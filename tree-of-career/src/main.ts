...
type NodeType = "root" | "branch" | "fruit";
...
const CONTENT: Record<string, SpecimenContent> = {
  root1: {
    kind: "Root I — Origin",
    title: "First Verses",
...
const FALL_TARGETS: Record<string, { dx: number; dy: number; rot: number }> = {
...
const panelKind = document.getElementById("panel-kind") as HTMLElement;
...
const panelReset = document.getElementById("panel-reset") as HTMLButtonElement;

let activeFruitKey: string | null = null;

function openPanel(content: SpecimenContent, showReset: boolean): void {
...
function clearActiveOfType(type: NodeType): void {
  document
    .querySelectorAll<SVGGElement>(`.node[data-type="${type}"].active`)
    .forEach((el) => el.classList.remove("active"));
  document
    .querySelectorAll<SVGPathElement>(`.${type}-path.lit`)
    .forEach((el) => el.classList.remove("lit"));
}

function litPathFor(type: NodeType, key: string): void {
...
function handleRootOrBranchClick(node: SVGGElement, type: NodeType, key: string): void {
...
function dropFruit(unit: HTMLElement, node: SVGGElement, key: string): void {
...
function resetFruit(): void {
  ...
  if (unit) {
    ...
    const node = unit.querySelector<SVGGElement>(".fruit-node");
...
function init(): void {
  ...
  nodes.forEach((node) => {
    const type = node.dataset.type as NodeType | undefined;
    const key = node.dataset.key;
    if (!type || !key) return;

    const activate = (): void => {
      if (type === "fruit") {
        const unit = node.closest<HTMLElement>(".fruit-unit");
        if (unit) dropFruit(unit, node, key);
      } else {
